...
func newProduct(db *gorm.DB, opts ...gen.DOOption) product {
	...
	_product.Longdesc = field.NewString(tableName, "longdesc")
	_product.Image = field.NewString(tableName, "image")
	_product.Sliceoptions = field.NewString(tableName, "sliceoptions")
	_product.CreatedBy = field.NewString(tableName, "created_by")
	_product.Admin = productHasOneAdmin{
...
func (p *product) updateTableName(table string) *product {
	...
	p.Longdesc = field.NewString(table, "longdesc")
	p.Image = field.NewString(table, "image")
	p.Sliceoptions = field.NewString(table, "sliceoptions")
...
func (p *product) fillFieldMap() {
	...
	p.fieldMap["longdesc"] = p.Longdesc
	p.fieldMap["image"] = p.Image
	p.fieldMap["sliceoptions"] = p.Sliceoptions
...
type productHasOneAdmin struct {
	db *gorm.DB

	field.RelationField
}

func (a productHasOneAdmin) Where(conds ...field.Expr) *productHasOneAdmin {
	if len(conds) == 0 {
		return &a
	}
...
func (a productHasOneAdmin) WithContext(ctx context.Context) *productHasOneAdmin {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a productHasOneAdmin) Session(session *gorm.Session) *productHasOneAdmin {
	a.db = a.db.Session(session)
	return &a
}

func (a productHasOneAdmin) Model(m *model.Product) *productHasOneAdminTx {
	return &productHasOneAdminTx{a.db.Model(m).Association(a.Name())}
}

func (a productHasOneAdmin) Unscoped() *productHasOneAdmin {
	a.db = a.db.Unscoped()
	return &a
}

type productHasOneAdminTx struct{ tx *gorm.Association }

func (a productHasOneAdminTx) Find() (result *model.Admin, err error) {
	return result, a.tx.Find(&result)
}

func (a productHasOneAdminTx) Append(values ...*model.Admin) (err error) {
...
func (a productHasOneAdminTx) Replace(values ...*model.Admin) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a productHasOneAdminTx) Delete(values ...*model.Admin) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a productHasOneAdminTx) Clear() error {
	return a.tx.Clear()
}

func (a productHasOneAdminTx) Count() int64 {
	return a.tx.Count()
}
